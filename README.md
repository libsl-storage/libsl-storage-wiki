...
Требуется реализовать отображения для просмотра спецификации, хранящейся на стороне BE.

Отображение должно содержать секцию мета-информации об отображаемой спецификации. В секции с мета-информацией должны быть отображены следующие данные о спецификации:

- название специфицируемой библиотеки;

- краткое описание специфицируемой библиотеки;

- список тэгов;

- список релизов.

Элементы списка релизов должны быть интерактивными и определять содержание секций сигнатур функций, графического представлений конечных автоматов и исходного текста спецификации libsl. 

Отображение должно содержать секцию wiki-описания спецификации в формате markdown, если отображаемая спецификация ее имеет.

Отображение должно содержать секцию с перечнем сигнатур функций, описанных в исходном тексте спецификации, с кратким описанием назначения, параметров, допустимых состояний для вызова и переходов, внешних эффектов (изменение глобальных переменных). Также в секции должно быть текстовое поле для фильтрации сигнатур по названию.

Отображение должно содержать графические представления конечных автоматнов, описанных в исходном тексте спецификации.

Отображение должно содержать исходный текст спецификации libsl. Требуется обеспечить возможность конфигурировать подсветку синтаксисических конструкций текста спецификации. 

Отображение должно содержать панель управления, предоставляющую следующие элементы:

- кнопка для навигации к отображению изменения спецификации;

- кнопка для удаления отображаемой спецификации.

Содержание панели управления должно определяться на стороне BE в зависимости текущего пользователя.
